...
ipfs.util.addFromFs(path, { recursive: true }).then((response) => {
    ...
    console.log('\n\n\n')
    console.log(`To point an .eth domain to this website, use this hash as value:`)
...
function printTable(array) {
    ...
    const sp = '\t'
    console.log(Object.keys(array[0]).map(key => key.padStart(maxLen[key])).join(sp))
    console.log(Object.keys(array[0]).map(key => ''.padStart(maxLen[key], '-')).join(sp))
    array.forEach(elem => {
        console.log(Object.keys(elem).map(key => String(elem[key]).padStart(maxLen[key])).join(sp))
    })
}